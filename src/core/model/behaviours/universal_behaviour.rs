...
use crate::{core::{LivingCreature, Faction, NavigationTarget, NavigationResult, model::living_creature, HiveMap, BeeType, EnemyType, RigidBody}, utils::FlatProvider};
...
pub fn fight_system(
    mut bees: Query<(&mut LivingCreature, &RigidBody, &Transform, &NavigationTarget), Without<EnemyType>>,
    mut enemies: Query<(&mut LivingCreature, &RigidBody, &Transform, &NavigationTarget), With<EnemyType>>,
) {
    for (mut bee_creature, _, bee_transform, bee_target) in bees.iter_mut() {
        if let NavigationTarget::Entity(e, _) = bee_target {
            if let Ok((mut enemy_creature, enemy_rb, enemy_transform, _)) = enemies.get_mut(*e) {
                let dist_sqr = bee_transform.flat().distance_squared(enemy_transform.flat());
                ...
                if dist_sqr < bee_attack_radius.powi(2) && bee_creature.can_attack() {
                    bee_creature.attack(&mut enemy_creature, enemy_transform.flat() - bee_transform.flat());
                }
            }
        }
    }
    ...
    for (mut enemy_creature, _, enemy_transform, enemy_target) in enemies.iter_mut() {
        if let NavigationTarget::Entity(e, _) = enemy_target {
            if let Ok((mut bee_creature, bee_rb, bee_transform, _)) = bees.get_mut(*e) {
                let dist_sqr = enemy_transform.flat().distance_squared(bee_transform.flat());
                ...
                if dist_sqr < enemy_attack_radius.powi(2) && enemy_creature.can_attack() {
                    enemy_creature.attack(&mut bee_creature, bee_transform.flat() - enemy_transform.flat());
                }
            }
        }
    }
...
pub fn universal_behaviour_system(
    mut behaviours: Query<(Entity, &mut UniversalBehaviour, &LivingCreature, &Transform, &Faction, &mut RigidBody, &mut NavigationTarget, &NavigationResult)>,
    ...
    for (e, mut behaviour, creature, transform, faction, mut rb, mut navigation, result) in behaviours.iter_mut() {
        if creature.is_dead() {
            continue;
        }
        ...
        let should_refresh = match *navigation {
            ...
            NavigationTarget::Position(_) => result.is_reached() || behaviour.time_since_last_refresh > 1.0,
        ...
        for (other_e, other_creature, other_t, other_faction) in all.iter() {
            ...
            if other_faction != faction {
                if dist_sqr < nearest_enemy_dist_sqr && other_t.flat().length_squared() < behaviour.enemy_attack_distance_to_hive.powi(2) {
                    nearest_enemy_dist_sqr = dist_sqr;
                    nearest_enemy = Some(other_e);
                }
            } else {
                if dist_sqr < behaviour.alert_distance.powi(2) && other_creature.time_since_last_damage_taken < 1.0 {
                    is_alert = true;
                }
            }
        }

        let enemy_attack_radius = if is_alert { behaviour.enemy_attack_radius_if_alerted} else {behaviour.enemy_attack_radius};
        ...
        if nearest_enemy_dist_sqr.sqrt() < enemy_attack_radius && nearest_enemy.is_some() {
            ...
            *navigation = NavigationTarget::Entity(nearest_enemy.unwrap(), creature.attack_radius);
            ...
            let refresh_wonder = if let NavigationTarget::Position(_) = *navigation {result.is_reached() || rb.stuck_tick > 5 } else {true};

            if refresh_wonder {
                rb.stuck_tick = 0;
                // wonder around then...
                for _ in 0..16 {
                    let x = rng.gen_range(-behaviour.max_wonder_distance_to_hive..behaviour.max_wonder_distance_to_hive);
                    let y = rng.gen_range(-behaviour.max_wonder_distance_to_hive..behaviour.max_wonder_distance_to_hive);
                    ...
                    || pos.length_squared() < behaviour.min_wonder_distance_to_hive.powi(2)
                        ...
                        || transform.flat().distance_squared(pos) < behaviour.min_wonder_distance.powi(2)
                    {
                        continue;
                    }
